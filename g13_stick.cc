...
G13_Stick::G13_Stick(G13_Device& keypad)
    : _keypad(keypad), _bounds(0, 0, 255, 255), _center_pos(127, 127), _north_pos(127, 0) {
    _stick_mode = STICK_KEYS;

    auto add_zone = [this, &keypad](const std::string& name, double x1, double y1, double x2,
                                    double y2) {
        _zones.push_back(G13_StickZone(*this, "STICK_" + name, G13_ZoneBounds(x1, y1, x2, y2),
...
G13_StickZone::G13_StickZone(G13_Stick& stick,
...
void G13_Stick::parse_joystick(unsigned char* buf) {
    ...
    G13_DBG("x=" << _current_pos.x << " y=" << _current_pos.y << " dx=" << dx << " dy=" << dy);
